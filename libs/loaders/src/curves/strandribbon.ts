/**
 * Turns strand control points into camera-agnostic triangle ribbons.
 *
 * @remarks
 * Shared by every curve-based hair importer, because the container a groom
 * arrives in says nothing about how it should be tessellated. The ribbon is
 * built in the strand's own frame: the quad expands along a per-segment side
 * vector that is perpendicular to the strand direction, which keeps the geometry
 * usable from any view without a GPU expansion pass.
 *
...
import type { AssetPrimitiveInfo } from '@zephyr3d/scene';
import type { StrandCurveSet } from './types';

/** Options controlling how strand curves are turned into renderable geometry. @public */
export type StrandRibbonOptions = {
  /**
...
/** {@link StrandRibbonOptions} with every default filled in. @public */
export type ResolvedStrandRibbonOptions = Required<
  Omit<StrandRibbonOptions, 'segmentsPerStrand' | 'maxStrands'>
> & {
...
/**
 * Fills in the defaults for a set of ribbon options.
 *
 * @param options - Caller-supplied options, all optional.
 * @returns The same options with every field resolved.
 * @public
 */
export function resolveStrandRibbonOptions(options?: StrandRibbonOptions): ResolvedStrandRibbonOptions {
  return {
...
/**
 * Builds a triangle-ribbon primitive for one strand set.
 *
 * @param curve - Decoded strands to tessellate.
 * @param opt - Resolved tessellation options.
 * @returns The primitive, or null when no strand survived filtering.
 * @public
 */
export function tessellateStrandRibbons(
  curve: StrandCurveSet,
  opt: ResolvedStrandRibbonOptions
): Nullable<AssetPrimitiveInfo> {
 ...
 * @internal
 */
function selectStrands(curve: StrandCurveSet, opt: ResolvedStrandRibbonOptions): SelectedStrand[] {
...
function strandHalfWidth(
  ...
  // Width is a diameter in the source's own linear unit.
 ...
 *
 * @remarks
 * Curve archives store cubic B-spline control points, but for ribbon geometry
 * the control polygon is close enough and far cheaper: strands carry 30 points
 * over a few centimetres, so the polygon and the spline differ by well under a
 * pixel.
 * @internal
 */
