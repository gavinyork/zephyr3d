...
export class PostWater extends AbstractPostEffect {
  ...
  constructor(elevation: number) {
    ...
    this._scatterGrad = new Interpolator(
      ...
      new Float32Array([0, 0, 0, 0.08, 0.41, 0.34, 0.13, 0.40, 0.45, 0.21, 0.5, 0.6]));
    this._rampTex = null;
  ...
  private _getWaterMesh(ctx: DrawContext) {
    ...
    if (!waterMesh) {
      ...
      waterMesh = new WaterMesh(device, {
        ...
        shading(scope: PBInsideFunctionScope, worldPos: PBShaderExp, worldNormal: PBShaderExp, foamFactor: PBShaderExp) {
          const pb = scope.$builder;
          ...
          pb.func('waterShading', [pb.vec3('worldPos'), pb.vec3('worldNormal'), pb.float('foamFactor')], function(){
