...
export type PBRTextureNames = ['occlusion','cheenColor','sheenRoughness','clearcoatIntensity','clearcoatNormal','clearcoatRoughness'];
export type PBRToMixedTextureType<T> = T extends [infer First, ...infer Rest]
  ? [First extends string ? ReturnType<typeof mixinTextureProps<First>> : never, ...PBRToMixedTextureType<Rest>] : [];

export type TT = ReturnType<typeof applyMaterialMixins<PBRToMixedTextureType<PBRTextureNames>,{ new(...args: any[]): IMeshMaterial }>>;
...
function mixinPBR<T extends IMeshMaterial>(BaseCls: { new(...args: any[]): T }) {
  ...
  const S = applyMaterialMixins(BaseCls as { new(...args: any[]): IMeshMaterial },
  ...
  return class extends S {
    ...
    applyUniformValues(bindGroup: BindGroup, ctx: DrawContext): void {
      ...
      if (this.needFragmentColor(ctx)) {
        ...
        if (this.occlusionTexture) {
          bindGroup.setValue('kkOcclusionStrength', this._occlusionStrength < 0 ? 0 : this._occlusionStrength > 1 ? 1 : this._occlusionStrength);
        }
        if (this.sheen) {
    ...
    calculateClearcoatFactor(scope: PBInsideFunctionScope): PBShaderExp {
      ...
      const clearcoatIntensityFactor = this.clearcoatIntensityTexture
        ? pb.textureSample(this.getClearcoatIntensityTextureUniform(scope), this.getClearcoatIntensityTexCoord(scope)).r
        : pb.float(1);
      const clearcoatRoughnessFactor = this.clearcoatRoughnessTexture
        ? pb.textureSample(this.getClearcoatRoughnessTextureUniform(scope), this.getClearcoatRoughnessTexCoord(scope)).g
        : pb.float(1);
      return pb.vec4(pb.mul(scope.kkClearcoatFactor.x, clearcoatIntensityFactor), pb.clamp(pb.mul(scope.kkClearcoatFactor.y, clearcoatRoughnessFactor), 0, 1), scope.kkClearcoatFactor.z, 1);
    }
    calculateClearcoatNormalAndNoV(scope: PBInsideFunctionScope, TBN: PBShaderExp, viewVec: PBShaderExp): PBShaderExp {
      const pb = scope.$builder;
      if (this.clearcoatNormalTexture) {
        const normalSample = pb.sub(pb.mul(pb.textureSample(this.getClearcoatNormalTextureUniform(scope), this.getClearcoatNormalTexCoord(scope)).rgb, 2), pb.vec3(1));
    ...
    calculateSpecularIBL(scope: PBInsideFunctionScope, brdf: PBShaderExp, f0: PBShaderExp, radiance: PBShaderExp, NoV: PBShaderExp, roughness: PBShaderExp, specularWeight: PBShaderExp): PBShaderExp {
      const pb = scope.$builder;
      const funcName = 'kkPBRCalcSpecularIBL';
      pb.func(funcName, [pb.vec4('brdf'), pb.vec3('f0'), pb.vec3('radiance'), pb.float('NdotV'), pb.float('roughness'), pb.float('specularWeight')], function(){
    ...
    calculateDiffuseIBL(scope: PBInsideFunctionScope, brdf: PBShaderExp, f0: PBShaderExp, diffuse: PBShaderExp, irradiance: PBShaderExp, NoV: PBShaderExp, roughness: PBShaderExp, specularWeight: PBShaderExp): PBShaderExp {
      const pb = scope.$builder;
