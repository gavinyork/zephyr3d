...
export class ScatteringLut {
  ...
  private static _aerialPerspectiveTextureWidth = this._aerialPerspectiveSliceX * this._aerialPerspectiveSliceZ;
  ...
  static getMultiScatteringLut() {
    ...
    if (!this._multiScatteringLut) {
      this.prepare(device);
      const format: TextureFormat = device.getDeviceCaps().textureCaps.supportHalfFloatColorBuffer && device.getDeviceCaps().textureCaps.supportLinearHalfFloatTexture
          ? 'rgba16f'
          : 'rgba8unorm';
  ...
  private static commonFunctions(this: PBGlobalScope) {
    ...
    pb.func('getScatteringValues', [pb.vec3('pos'), pb.vec3('rayleighScattering').out(), pb.float('mieScattering').out(), pb.vec3('extinction').out()], function(){
        ...
        fragment(pb) {
          ...
          pb.func('getValFromTLUT', [pb.vec3('pos'), pb.vec3('sunDir')], function(){
            ...
            this.$l.uv = pb.vec2(
              ...
              pb.max(0, pb.min(1, pb.div(pb.sub(this.height, that._groundRadiusMM), pb.sub(that._atmosphereRadiusMM, that._groundRadiusMM))))
            );
          ...
          pb.func('raymarchScattering', [pb.vec3('pos'), pb.vec3('rayDir'), pb.vec3('sunDir'), pb.float('tMax')], function(){
          ...
          pb.main(function(){
            ...
            */
            ...
            this.$l.extinction = pb.vec3(
              -((that._rayleighScatteringBase[0] + that._rayleighAbsorptionBase) * rayleighDensity + (that._mieScatteringBase + that._mieAbsorptionBase) * mieDensity),
              -((that._rayleighScatteringBase[1] + that._rayleighAbsorptionBase) * rayleighDensity + (that._mieScatteringBase + that._mieAbsorptionBase) * mieDensity),
              -((that._rayleighScatteringBase[2] + that._rayleighAbsorptionBase) * rayleighDensity + (that._mieScatteringBase + that._mieAbsorptionBase) * mieDensity),
            );
    ...
    if (!this._programMultiScatteringLut) {
      this._programMultiScatteringLut = device.buildRenderProgram({
        ...
        fragment(pb) {
          ...
          pb.func('getMultiScatteringValues', [pb.vec3('pos'), pb.vec3('sunDir'), pb.vec3('lumTotal').out(), pb.vec3('fms').out()], function(){
