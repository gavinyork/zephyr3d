...
export function createProgramHk(limit?: 4|2) {
  return Application.instance.device.buildRenderProgram({
    ...
    fragment(pb){
      ...
      const Spectrum = pb.defineStruct([Complex('dx'), Complex('hy'), Complex('dz'), Complex('sx'), Complex('sz'), Complex('dxdx'), Complex('dxdz'), Complex('dzdz')], 'Spectrum');
      ...
      pb.func('compressSpectrum', [Spectrum('spec'), pb.vec4('part0').out(), pb.vec4('part1').out()], function(){
