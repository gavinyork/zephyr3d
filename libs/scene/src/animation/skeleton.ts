...
export class SkinBinding extends Disposable {
  ...
  computeBoundingBox(info: SkinnedBoundingBox, invWorldMatrix: Matrix4x4) {
    ...
    for (let i = 0; i < info.boundingVertices.length; i++) {
      ...
      for (let j = 0; j < influenceCount; j++) {
        ...
        const matrix = this._jointMatrices[(Number(info.boundingVertexBlendIndices[base + j]) || 0) + this._jointOffsets[0] - 1];
        if (!matrix) {
          continue;
        }
   ...
   */
  getBoundingInfo(data: { positions: Float32Array; blendIndices: TypedArray; weights: TypedArray; influenceCount?: number }) {
    ...
    for (let i = 0; i < numVertices; i++) {
      ...
      for (let j = 0; j < influenceCount; j++) {
        ...
        const matrix = this._jointMatrices[(Number(data.blendIndices[base + j]) || 0) + this._jointOffsets[0] - 1];
        if (!matrix) {
          continue;
        }
    ...
    const info: SkinnedBoundingBox = {
      ...
      boundingVertexBlendIndices: new Float32Array(
        Array.from({ length: 6 * influenceCount }).map(
          (val, index) => data.blendIndices[indices[Math.floor(index / influenceCount)] * influenceCount + (index % influenceCount)]
        )
      ),
      boundingVertexJointWeights: new Float32Array(
        Array.from({ length: 6 * influenceCount }).map(
          (val, index) => data.weights[indices[Math.floor(index / influenceCount)] * influenceCount + (index % influenceCount)]
        )
      ),
  ...
  skinPositionsToLocal(
    ...
    const effectiveInfluenceCount = Math.max(
      1,
      influenceCount ?? Math.max(1, Math.floor(weights.length / Math.max(1, Math.floor(positions.length / 3))))
    );
