...
function buildNeighborData(
  ...
  for (let i = 0; i < vertexCount; i++) {
    const sorted = [...neighborMap[i].entries()].sort((a, b) => a[1] - b[1]).slice(0, maxNeighbors);
      ...
      pb.main(function () {
        ...
        this.$if(pb.lessThan(this.index, this.vertexCount), function () {
          ...
          this.$l.next = pb.add(
            this.current,
            this.velocity,
            pb.mul(this.gravity, pb.mul(this.dt2, this.freeWeight))
          );
          ...
          this.$if(pb.greaterThan(this.freeWeight, this.minDistance), function () {
            this.$for(pb.uint('i'), 0, this.sphereCount, function () {
              ...
              this.$if(pb.lessThan(this.lenCol, this.radius), function () {
                ...
                this.$l.normalStep = pb.mul(
                  this.contactNormal,
                  pb.dot(this.surfaceDelta, this.contactNormal)
                );
                ...
                this.$if(pb.greaterThan(this.tangentLen, this.minDistance), function () {
                  ...
                  this.$l.staticBasis = pb.max(
                    this.correctionLen,
                    pb.max(this.normalTravel, this.tangentLen)
                  );
      ...
      pb.main(function () {
        ...
        this.$if(pb.lessThan(this.index, this.vertexCount), function () {
          ...
          this.$if(pb.greaterThan(this.freeWeight, this.minDistance), function () {
            ...
            this.corrected = pb.add(
              pb.mul(this.corrected, this.freeWeight),
              pb.mul(this.rest, this.pinWeight)
            );
            this.$for(pb.uint('i'), 0, this.sphereCount, function () {
              this.$l.sphereBase = pb.mul(this.i, 4);
              this.$l.center = pb.vec3(
              ...
              this.$if(pb.lessThan(this.lenCol, this.radius), function () {
                this.$if(pb.greaterThan(this.lenCol, this.minDistance), function () {
                  this.corrected = pb.add(
                    this.center,
                    pb.mul(pb.div(this.deltaCol, this.lenCol), this.radius)
                  );
                }).$else(function () {
...
class GPUClothWrapBinding {
  ...
  static async createBindingData(
    ...
    const sourcePositionsInTargetSpace = transformPointArrayByMatrix(
      sourceToTargetBindMatrix,
      resolvedSourcePositions
    );
...
export class GPUClothSystem {
  ...
  constructor(options?: GPUClothSystemOptions) {
    ...
    const initialSimulationPositions =
      buildInitialSimulationPositions(options) ?? new Float32Array(options.positionData);
  ...
  private toCollisionSpaceRadius(worldRadius: number, axisWorld?: Vector3): number {
    ...
    if (axisWorld && axisWorld.magnitudeSq > 1e-6) {
      ...
      let tangent = Vector3.cross(
        Math.abs(axis.y) < 0.999 ? Vector3.axisPY() : Vector3.axisPX(),
        axis,
        new Vector3()
      );
      ...
      const localTangent = this.toCollisionSpaceVector(
        Vector3.scale(tangent, radius, new Vector3())
      ).magnitude;
      const localBitangent = this.toCollisionSpaceVector(
        Vector3.scale(bitangent, radius, new Vector3())
      ).magnitude;
  ...
  private updateSkinnedRestPositions(deltaTime = 0) {
    ...
    const skeletonId = String(target?.skeletonName ?? '');
    const skeleton =
      skeletonId && typeof target?.findSkeletonById === 'function'
        ? target.findSkeletonById(skeletonId)
        : null;
    if (skeleton?.skinPositionsToLocal && target?.invWorldMatrix) {
      const restPositions =
        this._dynamicRestPositionData &&
        this._dynamicRestPositionData.length === this._sourcePositionData.length
          ? this._dynamicRestPositionData
