        ...
        draw_list.IterateDrawCmds((draw_cmd: ImGui.DrawCmd): void => {
            if (draw_cmd.UserCallback !== null) {
                ...
                if (clip_rect.x < fb_width && clip_rect.y < fb_height && clip_rect.z >= 0.0 && clip_rect.w >= 0.0) {
                    ...
                    renderer.stream(vx, ixU16, indexOffset, draw_cmd.ElemCount, draw_cmd.TextureId, [scissorX, scissorY, scissorW, scissorH]);
                }
            }
...
export class Texture
{
    ...
    public Update(src:ImageBitmap|ImageData|HTMLCanvasElement|Uint8Array|Uint16Array, param?:any):void {
        let w, h;
        if(src==null)   {
            if(param) {
                w=param.width;
