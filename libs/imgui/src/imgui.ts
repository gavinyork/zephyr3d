...
function calculateSecionBoundsX(padding: number) {
  ...
  return [start + window.WindowPadding.x + padding, start + GetWindowWidth() - window.WindowPadding.x - padding];
}
...
export function BeginStatusBar() {
  ...
  const flags = ImGuiWindowFlags.NoDecoration|ImGuiWindowFlags.NoInputs|ImGuiWindowFlags.NoMove
    |ImGuiWindowFlags.NoScrollWithMouse|ImGuiWindowFlags.NoScrollbar|ImGuiWindowFlags.NoSavedSettings
    |ImGuiWindowFlags.NoBringToFrontOnFocus|ImGuiWindowFlags.NoBackground|ImGuiWindowFlags.MenuBar;
...
export function BeginSection(title: string) {
  ...
  PushClipRect(new ImVec2(boundsX[0], window.ClipRect.Min.y), new ImVec2(midPoint, window.ClipRect.Max.y), false);
  Text(title);
  PopClipRect();
  ...
  PushClipRect(new ImVec2(boundsX[0], window.ClipRect.Min.y), new ImVec2(boundsX[1], window.ClipRect.Max.y), false);
...
export function EndSection(borderSize?: number, rounding?: number) {
  ...
  if (borderSize > 0) {
    GetWindowDrawList().AddRect(
      ...
      rounding, ImDrawCornerFlags.All, borderSize);
  }
...
export class ImGuiWindow
{
    ...
    get ClipRect(): Bind.interface_ImRect { return this.native.ClipRect; }
    set ClipRect(v:Bind.interface_ImRect) { this.native.ClipRect.Min.Set(v.Min.x, v.Min.y); this.native.ClipRect.Max.Set(v.Max.x, v.Max.y); }
    get WorkRect(): Bind.interface_ImRect { return this.native.WorkRect; }
    set WorkRect(v:Bind.interface_ImRect) { this.native.WorkRect.Min.Set(v.Min.x, v.Min.y); this.native.WorkRect.Max.Set(v.Max.x, v.Max.y); }
    get InnerRect(): Bind.interface_ImRect { return this.native.InnerRect; }
    set InnerRect(v:Bind.interface_ImRect) { this.native.InnerRect.Min.Set(v.Min.x, v.Min.y); this.native.InnerRect.Max.Set(v.Max.x, v.Max.y); }
