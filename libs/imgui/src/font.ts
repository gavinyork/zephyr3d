...
export class TexturePage {
  constructor(tex_size: number, font: ImFont, fontScale: number) {
    ...
    this.PixelData = new Uint8Array(tex_size * tex_size * 4);
    ...
    this.Texture.Update(this.PixelData, { width: tex_size, height: tex_size });
  }
  ...
  Create(glyph: ImFontGlyph, ctx: CanvasRenderingContext2D): ImFontGlyph {
    ...
    glyph.TextureID = this.Texture._texture;
    ...
    for (let y = 0; y < image_size; y++) {
      for (let x = 0; x < image_size; x++) {
        let p = (py + y) * this.TextureSize * 4 + px * 4 + x * 4;
        this.PixelData[p++] = 0xff;
        this.PixelData[p++] = 0xff;
        this.PixelData[p++] = 0xff;
        this.PixelData[p++] = (img_data_u32[y * image_size + x] >> 24) & 0xff;
  ...
  UpdateTexture() {
    ...
    this.Texture.Update(this.PixelData);
  }

  get IsAvailable(): boolean {
    return this.Current < this.MaxCharCount;
  }
...
export class Font {
  ...
  Create(glyph: ImFontGlyph, font: ImFont, fontScale: number): ImFontGlyph {
    ...
    if (page == null) {
      page = new TexturePage(512, font, fontScale);
