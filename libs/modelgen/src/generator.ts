...
export type GeneratedModelSpec = {
  version?: number;
...
export type CoordinateRemap = 'none' | 'zUpToYUp' | 'yUpToZUp' | { axes?: [UVAxis, UVAxis, UVAxis] };
...
export type PositiveUVAxis = 'x' | 'y' | 'z';
export type UVAxis = PositiveUVAxis | '-x' | '-y' | '-z';
...
export type CylinderNode = BaseNode & {
  type: 'cylinder';
  radius?: number;
  height?: number;
  segments?: number;
...
export type SphereNode = BaseNode & {
  type: 'sphere';
  radius?: number;
...
export type RevolveNode = BaseNode & {
  type: 'revolve';
  profile: Vec2[];
  segments?: number;
  capTop?: boolean;
...
export type SurfaceNode = BaseNode & {
  type: 'surface';
...
export type CsgVertex = {
  pos: Vec3;
  normal: Vec3;
  uv: Vec2;
};

export type CsgPolygon = {
  vertices: CsgVertex[];
  plane: CsgPlane;
};

export type CsgPlane = {
  normal: Vec3;
  w: number;
};
...
export type WorkerPrimitivePayload = {
  // Explicitly backed by ArrayBuffer (never SharedArrayBuffer): these are always
  // freshly allocated here, and consumers that upload them to the GPU require the
  // narrower type.
  vertices: Record<string, { format: string; data: Float32Array<ArrayBuffer> }>;
...
/**
 * Called with a value in [0, 1] as generation proceeds.
 * @public
 */
...
/**
 * Tessellates a procedural model spec into interleaved vertex/index buffers.
 *
 * This is pure computation: it touches no graphics device, no DOM and no worker
 * globals, so it runs under plain node and is unit testable.
 *
 * @param spec - The procedural model spec. Field names are camelCase; use
 *   {@link normalizeGeneratedModelSpec} first if the source is snake_case.
 * @param deadlineAt - Absolute `Date.now()` deadline; generation throws once passed.
 *   Pass `Infinity` to disable the deadline.
 * @param onProgress - Optional progress callback, invoked with values in [0, 1].
 * @public
 */
export function generatePrimitive(
  ...
  // Saved and restored so nested/concurrent callers cannot clobber each other.
  const previousReporter = progressReporter;
  progressReporter = onProgress ?? null;
  try {
    return generatePrimitiveImpl(spec, deadlineAt);
  } finally {
    progressReporter = previousReporter;
  }
}

function generatePrimitiveImpl(spec: GeneratedModelSpec, deadlineAt: number): WorkerSuccess {
  ...
  const vertices: WorkerPrimitivePayload['vertices'] = {
    position: {
...
function applyUVSpec(
  ...
  // A non-default mode can only come from a spec, so `spec` is non-null in here.
  // The guard makes that invariant visible to the compiler rather than implied.
...
function postProgress(progress: number): void {
  progressReporter?.(Math.max(0, Math.min(1, progress)));
}
