...
export class ParallaxMapMaterial extends applyMaterialMixins(
  MeshMaterial,
  mixinAlbedoColor,
  mixinBlinnPhong
) {
  ...
  fragmentShader(scope: PBFunctionScope): void {
    ...
    if (this.needFragmentColor()) {
      ...
      pb.func('parallaxMapping', [pb.vec3('V'), pb.vec2('uv')], function () {
        ...
        if (that.mode === 'basic') {
          ...
          if (that.mode === 'steep') {
            ...
            this.$l.prevH = pb.add(
              pb.sub(that.sampleNormalMap(this, this.prevTCoords).a, this.currentLayerHeight),
              this.layerHeight
            );
            this.$l.weight = pb.div(this.nextH, pb.sub(this.nextH, this.prevH));
            this.currentTextureCoords = pb.add(
              pb.mul(this.prevTCoords, this.weight),
              pb.mul(this.currentTextureCoords, pb.sub(1, this.weight))
            );
            this.$return(this.currentTextureCoords);
          } else {
            this.$return(this.uv);
      ...
      pb.func(
        'calcUV',
        [pb.vec3('worldPos'), pb.vec3('worldNormal'), pb.vec3('viewPos'), pb.vec2('uv')],
        function () {
          ...
          this.$l.projVscr = pb.div(
            pb.vec2(pb.dot(this.vR1, this.viewPos), pb.dot(this.vR2, this.viewPos)),
            this.fDet
          );
          this.$l.projVtex = pb.vec3(
            pb.add(pb.mul(this.texDx, this.projVscr.x), pb.mul(this.texDy, this.projVscr.y)),
            pb.dot(this.worldNormal, this.viewPos)
          );
      ...
      scope.$l.texCoords = scope.calcUV(
        scope.$inputs.worldPos,
        scope.wNorm,
        pb.normalize(ShaderHelper.getCameraPosition(scope)),
        this.getNormalTexCoord(scope)
      );
