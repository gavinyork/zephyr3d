...
export class WoodMaterial extends applyMaterialMixins(MeshMaterial, mixinLambert) {
  ...
  constructor() {
    super();
    this._distored = new Vector3(8, 1.5, 8);
    this._darkcolor = new Vector3(0.43, 0.28, 0.1);
    this._lightcolor = new Vector3(0.6, 0.44, 0.2);
    this._density = 20;
  }
  ...
  applyUniformValues(bindGroup: BindGroup, ctx: DrawContext, pass: number): void {
    ...
    if (this.needFragmentColor(ctx)){
      ...
      bindGroup.setValue('density', this._density);
    }
  }
  ...
  fragmentShader(scope: PBFunctionScope): void {
    ...
    if (this.needFragmentColor()) {
      ...
      scope.lightColor = pb.vec3().uniform(2);
      scope.density = pb.float().uniform(2);
