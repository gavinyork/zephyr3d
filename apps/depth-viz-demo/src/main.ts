...
import { createDepthVisualizationModule } from './depth-visualization-module';
...
// Build a Forward+ pipeline and append the depth-visualization module. When
// present, the module takes over the final output; removing it restores the
// normal shaded frame. Both are pure user-side pipeline composition.
const depthModule = createDepthVisualizationModule();
...
btn.addEventListener('click', () => {
  ...
  btn.textContent = `Depth Visualization: ${depthEnabled ? 'ON' : 'OFF'}`;
