...
const ATTACK_ACTION_URL = 'https://cdn.zephyr3d.org/misc/attack.glb';
...
async function loadDemo() {
  ...
  const attack = await getEngine().resourceManager.fetchModel(ATTACK_ACTION_URL, scene);
  bot.animationSet.copyHumanoidAnimationFrom(
    attack.animationSet,
    attack.animationSet.getAnimationNames()[0],
    {
      targetName: 'attack'
    }
  );
...
function renderButtons() {
  ...
  if (ImGui.Button('Dispatch attack event') && controller) {
