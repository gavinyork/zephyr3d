...
草地笔刷绘制的是所选草层的**密度**：每一笔都会提高局部草地密度，引擎再从密度图确定性地生成实际草叶。笔刷`Strength`控制密度增长的速度，`Density`滑块设置该层满密度时单位面积的最大草叶数——想要更茂密的草地可以调大它。
...
如果草地刷的过于密集，可使用`erase grass`笔刷予以删除，操作方法和`grass`笔刷相同——它降低已绘制的密度而不是提高。
