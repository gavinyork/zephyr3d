...
`cheapRefractionDepth`（米）只在 `offset` 模式生效，表示廉价模式假设的水深，决定扭曲的强度。它刻意是一个定值而不是真实水深——按真实距离缩放会让穿出水面的物体出现第二份拷贝。
...
若水面**高于**池壁（例如水面在 y=4、池底在 y=0），焦散会整体偏移 `depth / tan(太阳仰角)`，在焦散边界与墙体阴影之间留出空白带。让水面贴着池沿、并让水面区域覆盖整片接收面，可使两者对齐。
...
> 与它所取代的大气雾一样，水下介质只作用于**不透明几何**——透明表面不会被它前方的水柱染色。
>
> 入水判定针对水体的**静止平面**而非位移后的水面，因此当摄像机与水面的距离在一个波高以内时，判定可能与实际波形不符。摄像机正好停在水线上时会二选一，而不是上下分屏。
...
调试时可把 `water.debugOutput` 设为 `'shoreFoam'` 单独查看这一层，或设为 `'waterDepth'` 查看它所依据的距离。
