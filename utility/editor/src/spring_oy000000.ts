...
export default class extends RuntimeScript<HostType> {
  ...
  onAttached(_host: HostType): void | Promise<void> {
    ...
    this.root = this.scene.findNodeByName('oy000000');
    for (const t of chains) {
      const chain = SpringChain.fromBoneChain(_host.findNodeByName(t[0]), _host.findNodeByName(t[1]), {
        damping: 0.9,
        stiffness: 0.82
      });
      const springSystem = new SpringSystem(chain, {
        ...
        maxPoseOffset: 0.18,
        maxPoseOffsetRoot: 0.12,
        maxPoseOffsetTip: 0.24
      });
