...
import { ClipmapTerrain, Mesh } from '@zephyr3d/scene';
import type { Camera } from '@zephyr3d/scene';
import { TerrainEditTool } from './terrain';
import { ClothPaintTool, findClothPaintHost } from './cloth';
...
export function isObjectEditable(obj: any): boolean {
  return obj instanceof ClipmapTerrain || !!findClothPaintHost(obj);
}
...
export function createEditTool(editor: Editor, obj: any, ctx: EditToolContext): EditTool {
  ...
  const clothHost = findClothPaintHost(obj);
  if (clothHost) {
    return new ClothPaintTool(editor, clothHost, ctx, obj instanceof Mesh ? obj : null);
  }
  return null;
}
