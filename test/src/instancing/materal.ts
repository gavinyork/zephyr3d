...
import { Application, DrawContext, MeshMaterial, ShaderHelper } from '@zephyr3d/scene';
...
export class LinearDepthMaterial extends MeshMaterial {
  ...
  fragmentShader(scope: PBFunctionScope): void {
    ...
    if (this.needFragmentColor()){
      ...
      scope.$l.linearDepth = pb.textureLoad(scope.linearDepthTex, pb.ivec2(scope.$builtins.fragCoord.xy), 0).r;
      this.outputFragmentColor(scope, scope.$inputs.worldPos, pb.vec4(pb.pow(pb.sub(1, scope.linearDepth),8), 0, 0, 1));
    } else {
