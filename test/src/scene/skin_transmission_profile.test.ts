import { SSSProfile } from '@zephyr3d/scene';
import { SSSMaterial } from '@zephyr3d/scene';
import {
...
describe('Skin transmission profile', () => {
  ...
  test('optical depth is calibrated against the table it indexes', () => {
    ...
    for (const metres of [0.002, 0.005, 0.01, 0.03]) {
      ...
      const opticalDepth = metres * SSS_OPTICAL_DEPTH_PER_WORLD_UNIT;
      ...
      const index = (opticalDepth / SSS_MAX_TRANSMISSION_OPTICAL_DEPTH) * (size - 1);
  ...
  test('a head-sized path saturates but a thin one does not', () => {
    ...
    const indexFor = (metres: number) =>
      ((metres * SSS_OPTICAL_DEPTH_PER_WORLD_UNIT) / SSS_MAX_TRANSMISSION_OPTICAL_DEPTH) * (size - 1);
